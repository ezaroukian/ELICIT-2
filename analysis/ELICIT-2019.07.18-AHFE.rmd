...
#check with ad.test (kSamples) manual bootstrapping - can't figure out how to shuffle unequal samples
#install.packages("kSamples")  
library(kSamples)
#ad.test(runif(50), rnorm(30))
ad.test(subset(dq.m.filtered, variable=="preference" & cond=="P")$value, subset(dq.m.filtered, variable=="preference" & cond=="M")$value)


data = cbind(p=subset(dq.m.filtered, variable=="preference" & cond=="P")$value, m= subset(dq.m.filtered, variable=="preference" & cond=="M")$value)
fc = function(d,i){
  d2 = d[i,]
  return(ad.test(d2$p, d2$m))
}

#install.packages("homtest")
library(homtest)
ADbootstrap.test(subset(dq.m.filtered, variable=="preference" & cond=="P")$value, subset(dq.m.filtered, variable=="preference" & cond=="M")$value)#needs equal samples

for( l in levels(dq.m.filtered$variable)){
  print(l)
  print(ad.test(subset(dq.m.filtered, variable==l & cond=="P")$value, subset(dq.m.filtered, variable==l & cond=="M")$value, method="simulated"))
  print("-----")
}
```

```{r}
#Verify TLX results with ANOVA

#Or just do KS and say that t-tests match

#KS
#install.packages("Matching")
library(Matching)
ad.ps = data.table(label=levels(dq.m.filtered$variable), p.val=0)
for( l in levels(dq.m.filtered$variable)){
  print(l)
  print(summary(ks.boot(subset(dq.m.filtered, variable==l & cond=="P")$value, subset(dq.m.filtered, variable==l & cond=="M")$value)))
  print(ad.test(subset(dq.m.filtered, variable==l & cond=="P")$value, subset(dq.m.filtered, variable==l & cond=="M")$value)$ad)
  print(t.test(subset(dq.m.filtered, variable==l & cond=="P")$value, subset(dq.m.filtered, variable==l & cond=="M")$value)$p.value)
  
  ggplot(subset(dq.m.filtered, variable=="mental"), aes(value, fill=cond)) +
    geom_histogram(alpha=0.5, position="identity") +
    scale_fill_manual(values=c("blue","gray"))
    theme_bw()
  
  print(median(subset(dq.m.filtered, variable==l & cond=="P")$value))
  print(median(subset(dq.m.filtered, variable==l & cond=="M")$value))
  print("-------")
  
  ad.ps[label==l,]$p.val = ks.boot(subset(dq.m.filtered, variable==l & cond=="P")$value, subset(dq.m.filtered, variable==l & cond=="M")$value)$ks.boot.pvalue
  
}
#Holm-Bonferroni correction for multiple comparisons
ad.ps = arrange(ad.ps, p.val)
ad.ps$rank = 1:nrow(ad.ps)
ad.ps$sig.holm = .05/(nrow(ad.ps)+1-ad.ps$rank)
ad.ps$sig = ad.ps$p.val < ad.ps$sig.holm


ggplot(dq.m.filtered, aes(value, fill=cond)) +
    geom_histogram(alpha=0.5, position="dodge", color="black") + #identity for overlay
    scale_fill_manual(values=c("navyblue","gray")) + 
    theme_bw() + facet_grid(.~variable)

ggplot(dq.m.filtered, aes(value, color=cond)) +
    stat_ecdf()+
    scale_color_manual(values=c("navyblue","darkgray"),
                       name="Condition",
                       breaks=c("P","M"),
                       labels=c("Plain", "Markup")) + 
    theme_bw() + 
    facet_wrap(.~variable, nrow=2, labeller=function(variable,value){return(list("mental"="1","physical"="2","temp"="3", "success"="4", "perf"="5", "stress"="6", "preference"="7")[value])}) + 
    ylab("Cumulative probability") + xlab("Response")



resampling <- function(n, data, test.statistic) {
  
  all.data <- data#c(data$experiment, data$control)
...
data = subset(dq.m.filtered, variable=="mental")$value
resampling(1000, 
           data, 
           function(x,y) mean(x) - mean(y))


t.test(subset(dq.m.filtered, variable=="mental" & cond=="P")$value, subset(dq.m.filtered, variable=="mental" & cond=="M")$value)
```
...
dtgg = gather(cbind(dplyr::select(dtg,worker,scenario,cond,q,correct),random=c(1/7,1/5,1/5,1/12,1/31,1/12,1/2)), q, correct, c(correct,random), factor_key=TRUE)
...
#demos.results = merge(demos,accTime[,c("worker","Count","RT")]) #I hope this is wrong, because the numbers don't match up
...
demos.results = merge(demos, accTime.summarise)
demos.results = merge(demos.results,dq.m[dq.m$variable=="preference",c("worker","value")], by="worker")
demos.results$prefSame = demos.results$value
demos.results[demos.results$Condition=="Plain","prefSame"] = 22 - demos.results[demos.results$Condition=="Plain","value"] #flip scale

demos.results.f = merge(demos, accTime.filtered.summarise)
demos.results.f = merge(demos.results.f,dq.m.filtered[dq.m.filtered$variable=="preference",c("worker","value")], by="worker")#doesn't have preference yet
demos.results.f$prefSame = demos.results.f$value
demos.results.f[demos.results.f$Condition=="Plain","prefSame"] = 22 - demos.results.f[demos.results.f$Condition=="Plain","value"] #flip scale

#Do you consider English your first or primary language? 
table(demos.results$language)
#Please select your age range: 
table(demos.results$age)
#Please select your gender: 
table(demos.results$gender)
#Please select your highest level of educational attainment: 
table(demos.results$education)

table(demos.results$occupation)
...
ggplot(demos.results.f, aes(value, as.numeric(occupation=="Science and Technology"))) +
  geom_jitter(height=.1) +
  geom_smooth(method="lm")+ facet_grid(~Condition)
with(demos.results.f, cor(value, occupation=="Science and Technology"))#teeny tiny
  with(subset(demos.results.f, Condition=="Plain"), cor(value, occupation=="Science and Technology"))
  with(subset(demos.results.f, Condition=="Markup"), cor(value, occupation=="Science and Technology"))
  
with(demos.results.f, cor(meanCount, occupation=="Science and Technology"))#tiny
with(demos.results.f, cor(meanRT, occupation=="Science and Technology"))#tiny
...
ggplot(demos.results.f, aes(meanCount, as.numeric(occupation=="Science and Technology"))) +
  geom_jitter(height=.1, width=.1, alpha=.2) +
  geom_smooth(method="lm") + facet_grid(~Condition)
...
cor(demos.results.f$value[-86], as.numeric(demos.results.f$gender[-86]))
cor(demos.results$value[-120], as.numeric(demos.results$gender[-120]))


#Filtered or unfiltered (probably filtered is fairest, otherwise RT is more looking to predict slackers)?
demos.results[-120,] %>% group_by(Condition) %>% summarise(count=cor(as.numeric(gender),meanCount), RT=cor(as.numeric(gender),meanRT), pref=cor(as.numeric(gender),value))
demos.results %>% group_by(Condition) %>% summarise(count=cor(as.numeric(age),meanCount), RT=cor(as.numeric(age),meanRT), pref=cor(as.numeric(age),value))
demos.results %>% group_by(Condition) %>% summarise(count=cor(as.numeric(education),meanCount), RT=cor(as.numeric(education),meanRT), pref=cor(as.numeric(education),value))
demos.results %>% group_by(Condition) %>% summarise(count=cor(as.numeric(occupation=="Science and Technology"),meanCount), RT=cor(as.numeric(occupation=="Science and Technology"),meanRT), pref=cor(as.numeric(occupation=="Science and Technology"),value))


demos.results.f %>% group_by(Condition) %>% summarise(count=cor(as.numeric(language),meanCount), RT=cor(as.numeric(language),meanRT), pref=cor(as.numeric(language),value))
demos.results.f[-86,] %>% group_by(Condition) %>% summarise(count=cor(as.numeric(gender),meanCount), RT=cor(as.numeric(gender),meanRT), pref=cor(as.numeric(gender),value))
with(demos.results.f[-86,], cor(as.numeric(gender), prefSame))
demos.results.f %>% group_by(Condition) %>% summarise(count=cor(as.numeric(age),meanCount), RT=cor(as.numeric(age),meanRT), pref=cor(as.numeric(age),value))
demos.results.f %>% group_by(Condition) %>% summarise(count=cor(as.numeric(education),meanCount), RT=cor(as.numeric(education),meanRT), pref=cor(as.numeric(education),value))
demos.results.f %>% group_by(Condition) %>% summarise(count=cor(as.numeric(occupation=="Science and Technology"),meanCount), RT=cor(as.numeric(occupation=="Science and Technology"),meanRT), pref=cor(as.numeric(occupation=="Science and Technology"),value)) 


ggplot(demos.results.f[-86], aes(value, gender)) +
  geom_jitter(height=.1, width=.1, alpha=.2) +
  #geom_smooth(method="lm") + 
  theme_bw() + ylab("Gender") + xlab("Preference (1=Plain, 21=Markup)")

ggplot(demos.results, aes(value, age)) +
  geom_jitter(height=.1, width=.1, alpha=.2) +
  #geom_smooth(method="lm") + 
  theme_bw() + ylab("Age") + xlab("Preference (1=Plain, 21=Markup)")

ggplot(demos.results, aes(value, education)) +
  geom_jitter(height=.1, width=.1, alpha=.2) +
  #geom_smooth(method="lm") + 
  theme_bw() + ylab("Education") + xlab("Preference (1=Plain, 21=Markup)")


ggplot(demos.results, aes(value, occupation)) +
  geom_jitter(height=.1, width=.1, alpha=.2) +
  #geom_smooth(method="lm") + 
  theme_bw() + ylab("Occupation") + xlab("Preference (1=Plain, 21=Markup)")



ggplot(demos.results.f[-86], aes(meanCount, gender, color=Condition)) +
  geom_jitter(height=.1, width=.1, alpha=.2) +
  #geom_smooth(method="lm") + 
  theme_bw() + ylab("Gender") + xlab("Accuracy count")

```

```{r}
#Correlations with accuracy, RT, preference


#Language
ggplot(demos.results.f, aes(meanCount, as.numeric(language))) +
  geom_jitter(height=.1) +
  geom_smooth(method="lm")+ facet_grid(~Condition) +
  ylab("Language") + xlab("Accuracy") + theme_bw()
#with(demos.results.f, cor(meanCount, language=="Yes"))
#  with(subset(demos.results.f, Condition=="Plain"), cor(meanCount, language=="Yes"))
#  with(subset(demos.results.f, Condition=="Markup"), cor(meanCount, language=="Yes"))

ggplot(demos.results.f, aes(meanRT, as.numeric(language))) +
  geom_jitter(height=.1) +
  geom_smooth(method="lm")+ facet_grid(~Condition) +
  ylab("Language") + xlab("RT") + theme_bw()
#with(demos.results.f, cor(meanRT, language=="Yes"))
#  with(subset(demos.results.f, Condition=="Plain"), cor(meanRT, language=="Yes"))
#  with(subset(demos.results.f, Condition=="Markup"), cor(meanRT, language=="Yes"))

ggplot(demos.results.f, aes(value, as.numeric(language))) +
  geom_jitter(height=.1) +
  geom_smooth(method="lm")+ facet_grid(~Condition) +
  ylab("Language") + xlab("Preference") + theme_bw()
#with(demos.results.f, cor(value, language=="Yes"))
#  with(subset(demos.results.f, Condition=="Plain"), cor(value, language=="Yes"))
#  with(subset(demos.results.f, Condition=="Markup"), cor(value, language=="Yes"))
...
ggplot(demos.results.f, aes(meanCount, as.numeric(language))) +
  geom_jitter(height=.1) +
  geom_smooth(method="lm")+ facet_grid(~Condition) +
  ylab("Gender") + xlab("Accuracy") + theme_bw()
#with(demos.results.f, cor(meanCount, language=="Yes"))
#  with(subset(demos.results.f, Condition=="Plain"), cor(meanCount, language=="Yes"))
#  with(subset(demos.results.f, Condition=="Markup"), cor(meanCount, language=="Yes"))
...
ggplot(demos.results.f, aes(value, as.numeric(language))) +
  geom_jitter(height=.1) +
  geom_smooth(method="lm")+ facet_grid(~Condition) +
  ylab("Gender") + xlab("Preference") + theme_bw()
#with(demos.results.f, cor(value, language=="Yes"))
#  with(subset(demos.results.f, Condition=="Plain"), cor(value, language=="Yes"))
#  with(subset(demos.results.f, Condition=="Markup"), cor(value, language=="Yes"))
...
#Age
ggplot(demos.results.f, aes(meanCount, as.numeric(age))) +
  geom_jitter(height=.1) +
  geom_smooth(method="lm")+ facet_grid(~Condition) +
  ylab("Age") + xlab("Accuracy") + theme_bw()
#with(demos.results.f, cor(meanCount, language=="Yes"))
#  with(subset(demos.results.f, Condition=="Plain"), cor(meanCount, as.numeric(age)))
#  with(subset(demos.results.f, Condition=="Markup"), cor(meanCount, as.numeric(age)))

ggplot(demos.results.f, aes(meanRT, as.numeric(age))) +
  geom_jitter(height=.1) +
  geom_smooth(method="lm")+ facet_grid(~Condition) +
  ylab("Age") + xlab("RT") + theme_bw()
#with(demos.results.f, cor(meanRT, language=="Yes"))
#  with(subset(demos.results.f, Condition=="Plain"), cor(meanRT, as.numeric(age)))
#  with(subset(demos.results.f, Condition=="Markup"), cor(meanRT, as.numeric(age)))

ggplot(demos.results.f, aes(value, as.numeric(age))) +
  geom_jitter(height=.1) +
  geom_smooth(method="lm")+ facet_grid(~Condition) +
  ylab("Age") + xlab("Preference") + theme_bw()
#with(demos.results.f, cor(value, language=="Yes"))
#  with(subset(demos.results.f, Condition=="Plain"), cor(value, as.numeric(age)))
#  with(subset(demos.results.f, Condition=="Markup"), cor(value, as.numeric(age)))
  
  
#Education
ggplot(demos.results.f, aes(meanCount, as.numeric(education))) +
  geom_jitter(height=.1) +
  geom_smooth(method="lm")+ facet_grid(~Condition) +
  ylab("Education") + xlab("Accuracy") + theme_bw()
#with(demos.results.f, cor(meanCount, language=="Yes"))
#  with(subset(demos.results.f, Condition=="Plain"), cor(meanCount, as.numeric(education)))
#  with(subset(demos.results.f, Condition=="Markup"), cor(meanCount, as.numeric(education)))

ggplot(demos.results.f, aes(meanRT, as.numeric(education))) +
  geom_jitter(height=.1) +
  geom_smooth(method="lm")+ facet_grid(~Condition) +
  ylab("Education") + xlab("RT") + theme_bw()
#with(demos.results.f, cor(meanRT, language=="Yes"))
#  with(subset(demos.results.f, Condition=="Plain"), cor(meanRT, as.numeric(education)))
#  with(subset(demos.results.f, Condition=="Markup"), cor(meanRT, as.numeric(education)))

ggplot(demos.results.f, aes(value, as.numeric(education))) +
  geom_jitter(height=.1) +
  geom_smooth(method="lm")+ facet_grid(~Condition) +
  ylab("Education") + xlab("Preference") + theme_bw()
#with(demos.results.f, cor(value, language=="Yes"))
#  with(subset(demos.results.f, Condition=="Plain"), cor(value, as.numeric(education)))
#  with(subset(demos.results.f, Condition=="Markup"), cor(value, as.numeric(education)))
...
ggplot(demos.results.f, aes(meanCount, as.numeric(occupation=="Science and Technology"))) +
  geom_jitter(height=.1) +
  geom_smooth(method="lm")+ facet_grid(~Condition) +
  ylab("Occupation") + xlab("Accuracy") + theme_bw()
#with(demos.results.f, cor(meanCount, language=="Yes"))
#  with(subset(demos.results.f, Condition=="Plain"), cor(meanCount, as.numeric(occupation=="Science and Technology")))
#  with(subset(demos.results.f, Condition=="Markup"), cor(meanCount, as.numeric(occupation=="Science and Technology")))

ggplot(demos.results.f, aes(meanRT, as.numeric(occupation=="Science and Technology"))) +
  geom_jitter(height=.1) +
  geom_smooth(method="lm")+ facet_grid(~Condition) +
  ylab("Occupation") + xlab("RT") + theme_bw()
#with(demos.results.f, cor(meanRT, language=="Yes"))
#  with(subset(demos.results.f, Condition=="Plain"), cor(meanRT, as.numeric(occupation=="Science and Technology")))
#  with(subset(demos.results.f, Condition=="Markup"), cor(meanRT, as.numeric(occupation=="Science and Technology")))

ggplot(demos.results.f, aes(value, as.numeric(occupation=="Science and Technology"))) +
  geom_jitter(height=.1) +
  geom_smooth(method="lm")+ facet_grid(~Condition) +
  ylab("Occupation") + xlab("Preference") + theme_bw()
#with(demos.results.f, cor(value, language=="Yes"))
#  with(subset(demos.results.f, Condition=="Plain"), cor(value, as.numeric(occupation=="Science and Technology")))
#  with(subset(demos.results.f, Condition=="Markup"), cor(value, as.numeric(occupation=="Science and Technology")))

```


Regression testing occupation to predict performance?
```{r}
accTime.filtered.r = merge(accTime.filtered,demos.results.f)

accTime.filtered.r$occupation = droplevels(as.factor(accTime.filtered.r$occupation))
contrasts(accTime.filtered.r$occupation) = contr.sum(12) #maybe deviation coding isn't the best idea with such unbalanced groups

Acc.c<-glmer(Mean ~ Condition + (1|worker),  data = accTime.filtered.r)
  summary(Acc.c)
Acc.co <-glmer(Mean ~ Condition * occupation + (1|worker),  data = accTime.filtered.r)#not sure what to do about contrasts
  summary(Acc.o)
  
anova(Acc.c,Acc.co)


RT.c <-lmer(RT ~ Condition + (1|worker),  data = accTime.filtered.r)
  summary(RT.c)
RT.co <-lmer(RT ~ Condition * occupation + (1|worker),  data = accTime.filtered.r)#not sure what to do about contrasts
  summary(Acc.o)
...
dcs = dplyr::select(dc, worker, comments)
dcs$len = sapply(as.character(dcs$comments), nchar)
dcs = merge(dcs,dplyr::select(demos.results,worker, value), by="worker")
dcs = merge(dcs,accTime.summarise)
dcs.f = subset(dcs,!(worker %in% remove))


ggplot(demos.results, aes(meanCount, dcs$len)) + #make sure the workers match
  geom_jitter(height=.1, width=.1, alpha=.2) +
  geom_smooth(method="lm") + facet_grid(~Condition) + theme_bw()
cor(dcs$len, demos.results$meanCount)
cor(dcs$len, demos.results$value)

  with(subset(demos.results, Condition=="Plain"), cor(meanCount, value))
  with(subset(demos.results, Condition=="Markup"), cor(meanCount, value))
  
  with(demos.results, cor(meanCount,prefSame))
  ggplot(demos.results, aes(meanCount, value)) + 
    geom_jitter(height=.1, width=.1, alpha=.2) +
    geom_smooth(method="lm") + facet_grid(~Condition) + theme_bw() + ylab("Preference (1 = Plain, 21 = Markup)") + xlab("Accuracy count")
...
drsv = merge(drs, dplyr::select(demos.results,worker,occupation,meanCount,meanRT,value,prefSame), by="worker")
drsv$trustNeg = rowSums(dplyr::select(drsv,trust1,trust2,trust3,trust4,trust5 ))
drsv$trustPos = rowSums(dplyr::select(drsv,trust6,trust7,trust8,trust9,trust10,trust11,trust12 ))
drsv$trust = drsv$trustPos + (8*5 - drsv$trustNeg)

drsv.f =  subset(drsv,!(worker %in% remove))
...
dTr$trustNeg = rowSums(dplyr::select(dTr,trust1,trust2,trust3,trust4,trust5 ))
dTr$trustPos = rowSums(dplyr::select(dTr,trust6,trust7,trust8,trust9,trust10,trust11,trust12 ))


#filter

#Can pool negative (1-5) and positive (6-12) ?
ggplot(demos.results.f, aes(value, dTr$trustNeg)) +
  geom_jitter(height=.1, width=.1, alpha=.2) +
  geom_smooth(method="lm")
cor(demos.results.f$value, dTr$trustPos)
cor(demos.results.f$value, dTr$trustNeg)

ggplot(demos.results.f, aes(meanCount, dTr$trustPos)) +
  geom_jitter(height=.1, width=.1, alpha=.2) +
  geom_smooth(method="lm") + facet_wrap(~Condition) + theme_bw()

cor(demos.results.f$meanCount, dTr$trustPos)
cor(demos.results.f$meanCount, dTr$trustNeg)




#Check baseline trust


ggplot(drsv.f, aes(trust,value)) +
  geom_jitter(height=.1, width=.1, alpha=.2) +
  geom_smooth(method="lm") + facet_wrap(~cond) + theme_bw()
with(drsv.f, cor(trust,value))
  with(subset(drsv.f,cond=="P"), cor(trust,value))
  with(subset(drsv.f,cond=="M"), cor(trust,value))
with(drsv, cor(trustPos,value))
with(drsv, cor(trustNeg,value))

ggplot(drsv, aes(trust,meanCount)) +
  geom_jitter(height=.1, width=.1, alpha=.2) +
  geom_smooth(method="lm") + facet_wrap(~cond) + theme_bw()
with(drsv.f, cor(trust,meanCount))
with(drsv.f, cor(trust,meanRT))
with(drsv, cor(trustPos,meanCount))
with(drsv, cor(trustNeg,meanCount))
```
...
```{r}
#More ANOVA - ICCRTS 2019
accTimeNew.filtered = accTime.filtered
accTime.filtered = accTimeNew.filtered

#Acc, Time, cond, scen, worker
accTime.filtered
accTime.filtered$Order = as.factor(accTime.filtered$Order)
table(accTime.filtered$Condition,accTime.filtered$Scenario)
table(accTime.filtered$Order,accTime.filtered$Scenario)
#Add TLX?



#RM - might just need LR...

library("ggpubr")
ggboxplot(accTime.filtered, x = "Condition", y = "Count", color = "Scenario",
          palette = c("#00AFBB", "#E7B800"))
ggboxplot(accTime.filtered, x = "Condition", y = "RT", color = "Scenario",
          palette = c("#00AFBB", "#E7B800"))

ggboxplot(accTime.filtered, x = "Order", y = "Count", color = "Scenario",
          palette = c("#00AFBB", "#E7B800"))
ggboxplot(accTime.filtered, x = "Order", y = "RT", color = "Scenario",
          palette = c("#00AFBB", "#E7B800"))

library(car)
my_anova <- aov(Count ~ Condition * Scenario, data = accTime.filtered)
Anova(my_anova, type = "III")#unbalanced
my_anova_RT <- aov(RT ~ Condition * Scenario, data = accTime.filtered)
Anova(my_anova_RT, type = "III")#unbalanced


library(lmerTest)
summary(lmer(RT ~ Scenario * Order + (1|worker), data = accTime.filtered))
summary(lmer(RT ~ Scenario * Condition * Order + (1|worker), data = accTime.filtered))

summary(glmer(Count/7 ~ Scenario * Order + (1|worker), data = accTime.filtered, family=binomial))


accTime.filtered %>% 
  group_by(Scenario, Condition) %>%
  summarise(medianRT = median(RT), medianCount = median(Count))
...
my_anova <- aov(Count ~ Condition * Scenario * Order, data = accTime.filtered)
Anova(my_anova, type = "III")#unbalanced
my_anova_RT <- aov(RT ~ Condition * Scenario * Order, data = accTime.filtered)
Anova(my_anova_RT, type = "III")#unbalanced
...
```{r}
#Do you perform better if you prefer what you saw?
...
accTime.filtered.pref = merge(accTime.filtered, dataPref[,c("worker","preference")])

accTime.filtered.pref$Preference = "N/A"
accTime.filtered.pref[accTime.filtered.pref$preference >11,]$Preference = "Markup"
accTime.filtered.pref[accTime.filtered.pref$preference <11,]$Preference = "Plain"

accTime.filtered.pref$PreferenceBin = "N/A"
accTime.filtered.pref[accTime.filtered.pref$Condition == accTime.filtered.pref$Preference,]$PreferenceBin = "Same"
accTime.filtered.pref[accTime.filtered.pref$Condition != accTime.filtered.pref$Preference,]$PreferenceBin = "Different"
accTime.filtered.pref[accTime.filtered.pref$Preference == "N/A",]$PreferenceBin = "N/A"


accTime.filtered.pref.sum = 
  accTime.filtered.pref %>%
  group_by(PreferenceBin) %>% #or Condition, Preference
  summarise(N= n(), meanAcc = mean(Count), meanRT=mean(RT))

accTime.filtered.pref %>%
  group_by(Preference, Condition) %>% #or Condition, Preference
  summarise(N= n(), medAcc = median(Count), medRT=median(RT))

accTime.filtered.pref = subset(accTime.filtered.pref, Preference != "N/A")

wilcox.test(accTime.filtered.pref$Count~accTime.filtered.pref$PreferenceBin) 
wilcox.test(accTime.filtered.pref$RT~accTime.filtered.pref$PreferenceBin) 


ggplot(accTime.filtered.pref.sum, aes(PreferenceBin, meanAcc) )+
  geom_point() 

ggplot(accTime.filtered.pref.sum, aes(Condition, meanAcc) )+
  geom_point() + 
  facet_wrap(~PreferenceBin)
